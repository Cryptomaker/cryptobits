...
static const char *strDNSSeed[][2] = {
    {"Cryptobits", "ip 121.215.159.167"},
    {"Cryptobits Pool", "ip 46.4.74.197"},
    {"Cryptobits Instance 1", "ip 54.232.196.111"},
};
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. Cryptobits is probably already running."), addrBind.ToString().c_str());
        else
